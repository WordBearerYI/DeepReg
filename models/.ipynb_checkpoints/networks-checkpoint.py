...
class ObsFeat2D(nn.Module):
    ...
    def __init__(self, n_points, z_size, n_out=1024):
        super(ObsFeat2D, self).__init__()
        self.n_out = n_out
        k = 3
        self.z_size = z_size
        p = int(np.floor(k / 2)) + 2
        self.conv1 = nn.Conv1d(self.z_size +2, 64, kernel_size=k, padding=p, dilation=3)
    ...
    def forward(self, x):
        assert(x.shape[1] == self.z_size +2), "the input size must be <Bx2xL> "
...
class ObsFeatAVD(nn.Module):
    ...
    def __init__(self, z_size, n_out=1024):
        super(ObsFeatAVD, self).__init__()
        self.n_out = n_out
        k = 3
        self.z_size = z_size
        p = int(np.floor(k / 2)) + 2
        self.conv1 = nn.Conv2d(3 + self.z_size,64,kernel_size=k,padding=p,dilation=3)
    ...
    def forward(self, x):
        #assert(x.shape[1]==self.z_size+3),"the input size must be <Bx3xHxW> "
...
class LocNetReg2D(nn.Module):
    def __init__(self, n_points,z_size, out_dims):
        super(LocNetReg2D, self).__init__()
        self.obs_feat_extractor = ObsFeat2D(n_points,z_size)
...
class LocNetRegAVD(nn.Module):
    def __init__(self, z_size, out_dims):
        super(LocNetRegAVD, self).__init__()
        self.obs_feat_extractor = ObsFeatAVD(z_size)
        n_in = self.obs_feat_extractor.n_out
        self.fc = MLP([n_in, 512, 256, out_dims])
