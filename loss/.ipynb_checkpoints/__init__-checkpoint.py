...
def bce_ch(pred, targets, obs_global, valid_obs=None, bce_weight=None, seq=2, gamma=0.1):
    ...
    return gamma * bce_loss + (1 - gamma) * ch_loss, None

def bce_chmk(pred, targets, obs_global, valid_obs=None, bce_weight=None, seq=2, gamma=0.1, masks=None, epoch = 0):
    assert valid_obs is not None
    assert masks is not None
    bce_loss = bce(pred, targets, bce_weight)
    ch_loss, masks = chamfer_loss_dynamic_mask(obs_global, valid_obs,masks=masks, seq=seq, epoch = epoch)
